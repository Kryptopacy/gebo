...
const SYSTEM_PROMPT = [
  ...
  "",
  "Pages you can link (relative): / (home), /c/{slug} (category), /a/{tokenId} (agent card), /a/{tokenId}/hire (choose scope, simulate, then hire on chain with your own wallet), /search, /shortlist?ids={tokenId1},{tokenId2} (compare up to six agents side by side - no scores, no winner, the judgement stays with the user), /live (liveness ledger), /authority (what an agent could do to a wallet), /compare (counterfactual Agent Advantage Report), /methodology.",
  "",
