 ...
 * The liveness ledger.
 *
 * This page exists because of what other indexers cannot answer. They record
 * registration and a point-in-time health flag; none keeps a longitudinal record,
 * so none can say whether an agent has been reliable, or when it stopped
 * answering. That is the whole argument for GEBO producing this data rather than
 * reselling somebody else's.
 *
 * It is published rather than kept internal, because a measurement locked in one
 * vendor's database is that vendor's asset, not the ecosystem's.
 */
          ...
          <dl className="readouts">
            ...
            <div className="readout">
              ...
              <dd style={{ fontSize: "1.25rem" }}>
                {s.p50Ms ?? "—"}
                <span className="t-4" style={{ fontSize: "0.8rem" }}> / {s.p95Ms ?? "—"} ms</span>
              </dd>
              ...
              {transitions.map((t, i) => {
                ...
                return (
                  <div key={`${t.at}-${i}`} className="row" style={{ gridTemplateColumns: "7rem minmax(0,1.3fr) 1fr 9rem minmax(0,1fr)" }}>
                    ...
                    <div className="xs t-3">{t.detail ?? t.errClass ?? "—"}</div>
                  </div>
                );
              })}
            </div>
