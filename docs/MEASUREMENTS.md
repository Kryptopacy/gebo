...
> **Census measured at 2026-09-06 09:18:01 UTC.**
...
```
  336,677   identities minted
  336,677   censused                        100.00% of the registry
  255,228   registration file resolved      75.81% of censused
  255,220   carry a name                    100.00% of resolved
  250,151   self-declare "active": true     98.01% of resolved
   96,600   declare ANY service endpoint    37.85% of resolved
    3,175   callable and lint-clean         1.24% of resolved
```

> **153,551 identities declare themselves active while publishing no endpoint of any
> kind.** `"active": true` is self-reported, unverified, and asserted by
> 98.01% of resolved registrations against 37.85% that
...
`data:` pointers resolve from calldata with no network call. `10,554`
...
| --- | --- | --- | --- |
| Distinct owners | **272,514** | Distinct endpoint operators | **115** |
| Holding exactly one | 248,801 | Largest operator's share | **77.06%** |
| Top 10 owners combined | 5.98% | Top 5 operators | 98.83% |
| Mean per owner | 1.2 | Top 20 operators | 99.75% |

> **272,514 wallets registered an identity. 115 organisations run
...
> present 272,514 apparently independent suppliers.
...
| | |
| --- | --- |
| Probes recorded | 226,592 |
| Distinct endpoints probed | 1,432 |
| Completed a protocol handshake | 220,938 |
| Agents `DORMANT` | 20,556 |
| Agents `VERIFIED` | 480 |
| Agents `SHADOWED` | 206 |
| Agents `LISTED` | 36 |
