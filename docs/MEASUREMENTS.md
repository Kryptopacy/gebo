...
> **Census measured at 2026-08-30 15:03:00 UTC.**
...
```
  319,423   identities minted
  319,423   censused                        100.00% of the registry
  241,269   registration file resolved      75.53% of censused
  241,261   carry a name                    100.00% of resolved
  236,434   self-declare "active": true     98.00% of resolved
   83,740   declare ANY service endpoint    34.71% of resolved
    2,929   callable and lint-clean         1.21% of resolved
```

> **152,694 identities declare themselves active while publishing no endpoint of any
> kind.** `"active": true` is self-reported, unverified, and asserted by
> 98.00% of resolved registrations against 34.71% that
...
`data:` pointers resolve from calldata with no network call. `10,407`
...
| --- | --- | --- | --- |
| Distinct owners | **263,544** | Distinct endpoint operators | **107** |
| Holding exactly one | 243,497 | Largest operator's share | **80.62%** |
| Top 10 owners combined | 6.30% | Top 5 operators | 98.70% |
| Mean per owner | 1.2 | Top 20 operators | 99.76% |

> **263,544 wallets registered an identity. 107 organisations run
...
> present 263,544 apparently independent suppliers.
...
| | |
| --- | --- |
| Probes recorded | 138,991 |
| Distinct endpoints probed | 1,432 |
| Completed a protocol handshake | 135,224 |
| Agents `DORMANT` | 20,538 |
| Agents `VERIFIED` | 493 |
| Agents `SHADOWED` | 206 |
| Agents `LISTED` | 41 |
