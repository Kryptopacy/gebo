...
> **Census measured at 2026-08-26 01:34:02 UTC.**
...
```
  303,897   identities minted
  303,897   censused                        100.00% of the registry
  229,356   registration file resolved      75.47% of censused
  229,348   carry a name                    100.00% of resolved
  224,774   self-declare "active": true     98.00% of resolved
   72,521   declare ANY service endpoint    31.62% of resolved
    2,682   callable and lint-clean         1.17% of resolved
```

> **152,253 identities declare themselves active while publishing no endpoint of any
> kind.** `"active": true` is self-reported, unverified, and asserted by
> 98.00% of resolved registrations against 31.62% that
...
`data:` pointers resolve from calldata with no network call. `10,243`
...
| --- | --- | --- | --- |
| Distinct owners | **254,857** | Distinct endpoint operators | **107** |
| Holding exactly one | 238,668 | Largest operator's share | **87.28%** |
| Top 10 owners combined | 6.62% | Top 5 operators | 98.51% |
| Mean per owner | 1.2 | Top 20 operators | 99.72% |

> **254,857 wallets registered an identity. 107 organisations run
...
> present 254,857 apparently independent suppliers.
...
| | |
| --- | --- |
| Probes recorded | 79,861 |
| Distinct endpoints probed | 1,432 |
| Completed a protocol handshake | 77,651 |
| Agents `DORMANT` | 20,574 |
| Agents `VERIFIED` | 459 |
| Agents `SHADOWED` | 206 |
| Agents `LISTED` | 39 |
