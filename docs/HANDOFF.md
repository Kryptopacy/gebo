...
- **Rot audit beyond measurements** (same fix pattern, applied everywhere it
  applied): (1) `CENSUS_FALLBACK` in src/lib/data.ts is now GENERATED by the
  same script between markers (it was 11 days stale and wrong by 3 orders of
  magnitude on fatalDefects: typed 12, measured 12_238; the first generator
  version emitted comma numbers like `12,220` which TS parses as the expression
  `(12,220)`=220 - now underscore-grouped, and the Action runs tsc before
  committing). (2) `.env.example` rewritten to every var code actually reads
  (X402_PAY_TO and CRON_SECRET were undocumented; CHAIN_ID/ALTANA_* were dead)
  plus a readiness gate that scans `process.env.*` references against it -
  33 vars, all documented. (3) The four legacy GH workflows
  (sync/probe/opportunities/resolve) are workflow_dispatch-only now: pg_cron
  owns the cadence, and a restored Actions billing unlock would have
  double-scheduled the whole pipeline against third-party endpoints.
  Readiness is 27/27.
